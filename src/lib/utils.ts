...
export const getCategoryChain = cache(async (slugs: string[]): Promise<CategoryChainResult> => {
    let categories = [];
        ...
        for (const slug of slugs) {
            ...
            if (ctg) {
                categories.push({
                    ...
                    name: ctg.name,
                    slug: ctg.slug,
                    childrenCount: ctg.childrenCount,
                });
                isLastChild = ctg.childrenCount === 0;
            }
        }
    } else {
        const { data } = await getProductDetail({ slug: slugs[slugs.length - 1] });
...
export function hasValidData(value: any): boolean {
    ...
    if (Array.isArray(value)) {
        ...
        return value.some(item => hasValidData(item));
    }
...
export function parseSearchParams(searchParams: SearchParams): IProductFilterParams {
    ...
    if (searchParams.manufacturers) {
        filters.manufacturers = (searchParams.manufacturers as string).split(',');
    }

    // Origins
    if (searchParams.origins) {
        filters.origins = (searchParams.origins as string).split(',');
    }

    // Units
    if (searchParams.units) {
        filters.units = (searchParams.units as string).split(',');
    }

    // Tags
    if (searchParams.tags) {
        filters.tags = (searchParams.tags as string).split(',') as any;
    }

    // Price range
    if (searchParams.priceMin) {
        filters.priceMin = Number(searchParams.priceMin);
    }
    if (searchParams.priceMax) {
        filters.priceMax = Number(searchParams.priceMax);
    }

    // Weight range
    if (searchParams.weightMin) {
        filters.weightMin = Number(searchParams.weightMin);
    }
    if (searchParams.weightMax) {
        filters.weightMax = Number(searchParams.weightMax);
    }

    // Dimension ranges
    if (searchParams.lengthMin) filters.lengthMin = Number(searchParams.lengthMin);
    if (searchParams.lengthMax) filters.lengthMax = Number(searchParams.lengthMax);
    if (searchParams.widthMin) filters.widthMin = Number(searchParams.widthMin);
    if (searchParams.widthMax) filters.widthMax = Number(searchParams.widthMax);
    if (searchParams.heightMin) filters.heightMin = Number(searchParams.heightMin);
    if (searchParams.heightMax) filters.heightMax = Number(searchParams.heightMax);
    ...
    Object.entries(searchParams).forEach(([key, value]) => {
        if (key.startsWith('spec_') && value) {
            ...
            specifications[specName] = (value as string).split(',');
        }
    });
...
export function capitalizeFirstLetterEachWord(str: string) {
    ...
    for (const word of words) {
        newWord.push(word[0].toUpperCase() + word.slice(1).toLowerCase());
    }
    return newWord.join(' ');
}
