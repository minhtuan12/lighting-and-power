...
export function convertNestedCategories(
	...
	return categories.map((c) => ({
		...
		[childrenKey]: (c?.children
			? convertNestedCategories(c.children, labelKey, valueKey, childrenKey)
			: []) as any,
	}));
}
...
export const getCategoryChain = cache(
	async (slugs: string[]): Promise<CategoryChainResult> => {
		let categories = [];
...
export function parseSearchParams(
	...
	if (searchParams.lengthMin)
		filters.lengthMin = Number(searchParams.lengthMin);
	if (searchParams.lengthMax)
		filters.lengthMax = Number(searchParams.lengthMax);
	...
	if (searchParams.heightMin)
		filters.heightMin = Number(searchParams.heightMin);
	if (searchParams.heightMax)
		filters.heightMax = Number(searchParams.heightMax);
...
export const getProvinces = cache(
	async ({
		...
		try {
			...
			const res = await fetch(`${url}?depth=${depth}`, {
