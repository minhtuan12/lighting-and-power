export const messages = {
    vi: {
        error: "Đã có lỗi xảy ra",
        auth: {
            successful: "Đăng nhập thành công",
            required: "Vui lòng nhập email hoặc số điện thoại",
            generateTokens: 'Đã có lỗi xảy ra khi xác thực',
            invalid: "Tài khoản không đúng",
            inactive: "Tài khoản đã bị vô hiệu hóa",
            cannotChangeEmail: "Không thể thay đổi email",
            cannotChangePhone: "Không thể thay đổi số điện thoại",
            phoneExists: 'Số điện thoại đã tồn tại',
            mailExists: 'Email đã tồn tại',
            usernameExists: 'Tên tài khoản đã tồn tại',
        },
    },
    en: {
        error: "An error occurred, please try again later",
        auth: {
            successful: "Successfully login",
            required: "Email/phone and password are required",
            generateTokens: 'Failed to generate tokens',
            invalid: "Invalid email/phone or password",
            inactive: "Account has been deactivated",
            cannotChangeEmail: "You cannot change your email",
            cannotChangePhone: "You cannot change your phone number",
            phoneExists: 'Phone number already exists',
            mailExists: 'Email already exists',
            usernameExists: 'Username already exists',
        },
    }
}
