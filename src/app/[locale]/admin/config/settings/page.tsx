    ...
    const handleSubmit = async (values: any) => {
        try {
            ...
            let configData: Omit<IConfig, '_id'> = {
                ...
                banners: [...config?.banners?.filter((i: string) => imageFilesPreview.includes(i)), ...uploadedImageUrls],
            };
                        ...
                        <div className="space-y-4">
                            {imageFilesPreview.map((preview, idx) => (
                                <div key={idx} className="flex gap-2 mb-3">
                                    {preview ? (
                                        <Flex justify='space-between' align='center' className='w-full' gap={15}>
                                            <Flex align='center' gap={10}>
                                                <img src={preview} alt={`product-${idx}`} className="w-92 h-30 object-cover rounded" />
