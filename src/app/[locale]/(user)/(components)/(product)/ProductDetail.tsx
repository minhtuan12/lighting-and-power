        ...
        <Row gutter={30}>
            <Col span={8}>
                <Flex vertical className="w-full" gap={10}>
                    <BadgeRibbon text={t('authentic')} color="orange" placement="start">
                        <DefaultImage
                ...
                <Flex vertical gap={20}>
                    ...
                    <Row gutter={40}>
                        <Col span={12}>
                            ...
                            >
                                <div>{t('remaining')}: {product.stock} {product.unit}</div>
                            ...
                            <Flex vertical gap={12} className="!mt-[15px] w-full" justify="space-between">
                                <Flex gap={8} className="w-full" justify="space-between">
                                    <b>{t('quantity')}: </b>
                                    <span>{t('min')}: {product.minOrderQuantity}</span>
                        ...
                        </Col>
                        {
                            (product.priceTiers && product.priceTiers.length > 0) && <Col span={12}>
                                <Row gutter={6} className="border-y border-y-[#BDBDBD] w-full py-2">
                                    <Col span={12} className="text-center !text-[12px]">{t('quantity')} ({product.unit})</Col>
                                    <Col span={12} className="text-center !text-[12px]">{t('priceEach')} (VNĐ)</Col>
                                </Row>
                                {
...

        {
            (product.specifications && product.specifications.length > 0) && <Row className="w-full">
                <Col className="space-y-3 detail-product-table w-full">
                    <h3 className="text-[23px]">{t('filter.specifications')}</h3>
                    <Table
                        rowKey={'name'}
                        bordered
                        columns={specColumns}
