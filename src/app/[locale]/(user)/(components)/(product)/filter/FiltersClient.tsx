...
const FilterHeader = (
    <Flex gap={10}>
        <Icon src="/images/filter.png" size={15} />
        Lọc sản phẩm
    </Flex>
)
...
export default function FiltersClient({
    ...
    const [filterState, setFilterState] = useState<FilterState>(() => ({
        ...
        dimensionRanges: {
            length: [
                currentFilters.lengthMin ||
                    filterOptions.dimensionRanges?.length?.min ||
                    0,
                currentFilters.lengthMax ||
                    filterOptions.dimensionRanges?.length?.max ||
                    0,
            ],
            width: [
                currentFilters.widthMin ||
                    filterOptions.dimensionRanges?.width?.min ||
                    0,
                currentFilters.widthMax ||
                    filterOptions.dimensionRanges?.width?.max ||
                    0,
            ],
            height: [
                currentFilters.heightMin ||
                    filterOptions.dimensionRanges?.height?.min ||
                    0,
                currentFilters.heightMax ||
                    filterOptions.dimensionRanges?.height?.max ||
                    0,
            ],
        },
    ...
    const renderFilterContent = useCallback(
        (key: string, values: any) => {
            switch (key) {
                    ...
                    return (
                        ...
                        >
                            ...
                            <p className="text-xs text-gray-500 text-center">
                                {values.min.toLocaleString("vi-VN")}đ -{" "}
                                {values.max.toLocaleString("vi-VN")}đ
                            </p>
                        </Space>
                    )
                ...
                // ============= WEIGHT RANGE =============
                case "weightRange":
                    if (!values?.min || !values?.max || values.hasWeight === 0)
                        return null
                    ...
                    return (
                        ...
                        >
                            <div className="text-xs text-gray-500 mb-2">
                                {values.hasWeight} sản phẩm có thông tin trọng
                                lượng
                            </div>
                            <Slider
                                range
                                ...
                                onChange={(value) =>
                                    handleFilterChange("weightRange", value)
                                }
                                tooltip={{
                            ...
                            <Flex gap={8} align="center">
                                <InputNumber
                                    ...
                                    onChange={(value) =>
                                        handleFilterChange("weightRange", [
                                            value || values.min,
                                            filterState.weightRange[1],
                                        ])
                                    }
                                    addonAfter="g"
                                    className="flex-1"
                            ...
                            {values.length?.hasValue > 0 && (
                                <div>
                                    ...
                                    <Slider
                                        ...
                                        value={
                                            filterState.dimensionRanges.length
                                        }
                                        onChange={(value) =>
                                            handleFilterChange(
                                                "dimensionRanges",
                            ...
                            {values.width?.hasValue > 0 && (
                                <div>
                                    ...
                                    <Slider
                                        ...
                                        value={
                                            filterState.dimensionRanges.width
                                        }
                                        onChange={(value) =>
                                            handleFilterChange(
                                                "dimensionRanges",
                            ...
                            {values.height?.hasValue > 0 && (
                                <div>
                                    ...
                                    <Slider
                                        ...
                                        value={
                                            filterState.dimensionRanges.height
                                        }
                                        onChange={(value) =>
                                            handleFilterChange(
                                                "dimensionRanges",
                ...
                case "specifications":
                    ...
                    const specItems = values.map((spec) => ({
                        ...
                        children: (
                            <Checkbox.Group
                                value={
                                    filterState.specifications[spec.name] || []
                                }
                                onChange={(checkedValues) =>
    ...
    const buildURL = (filters: any) => {
        ...
        // Dimensions
        if (
            filters.dimensionRanges.length[0] !==
            filterOptions.dimensionRanges?.length?.min
        ) {
            params.set(
                "lengthMin",
                filters.dimensionRanges.length[0].toString(),
            )
        }
        if (
            filters.dimensionRanges.length[1] !==
            filterOptions.dimensionRanges?.length?.max
        ) {
            params.set(
                "lengthMax",
                filters.dimensionRanges.length[1].toString(),
            )
        }
        if (
            filters.dimensionRanges.width[0] !==
            filterOptions.dimensionRanges?.width?.min
        ) {
            params.set("widthMin", filters.dimensionRanges.width[0].toString())
        }
        if (
            filters.dimensionRanges.width[1] !==
            filterOptions.dimensionRanges?.width?.max
        ) {
            params.set("widthMax", filters.dimensionRanges.width[1].toString())
        }
        if (
            filters.dimensionRanges.height[0] !==
            filterOptions.dimensionRanges?.height?.min
        ) {
            params.set(
                "heightMin",
                filters.dimensionRanges.height[0].toString(),
            )
        }
        if (
            filters.dimensionRanges.height[1] !==
            filterOptions.dimensionRanges?.height?.max
        ) {
            params.set(
                "heightMax",
                filters.dimensionRanges.height[1].toString(),
            )
        }

        // Specifications
        Object.entries(filters.specifications).forEach(
            ([name, values]: any) => {
                if (values.length > 0) {
                    params.set(
                        `spec_${encodeURIComponent(name)}`,
                        values.join(","),
                    )
                }
            },
    ...
    const collapseItems = Object.entries(filterOptions)
        ...
        .map(([key, values]) => ({
            key,
            label: (
                <span className="text-[13px] font-medium">{getLabel(key)}</span>
            ),
    ...
    return (
        <Flex className="w-full" vertical gap={10}>
            ...
            <Flex gap={8} className="mt-6 px-4" justify="end">
                <Button onClick={handleResetFilters}>{t("clearAll")}</Button>
