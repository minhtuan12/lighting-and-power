...
export const SocialService = {
    ...
    async friendship(
        ...
        if (action === 'remove') {
            if (existing) {
                const update = {
                    ...
                    requesterId: existing.requesterId.toString(),
                    addresseeId: existing.addresseeId.toString(),
                    status: 'none',
                }
                await existing.deleteOne()
                emitToUser(
                    existing.requesterId.toString(),
                    'friendship:update',
                    update,
                )
                emitToUser(
                    existing.addresseeId.toString(),
                    'friendship:update',
                    update,
                )
            }
        ...
        if (action === 'accept')
            await NotificationService.createFriendAccepted(
                userId,
                existing.requesterId.toString(),
            )
        return serialize(existing)
    ...
    async getOrCreateConversation(userId: string, otherId: string) {
        ...
        if (!conversation)
            conversation = await Conversation.create({ participantIds: ids })
    ...
    async listConversations(userId: string) {
        ...
        const conversations: any[] = await Conversation.find({
            ...
            .populate('participantIds', 'fullName username avatar')
            .sort({ updatedAt: -1 })
            .lean()
        const result = await Promise.all(
            conversations.map(async (conversation) => {
                ...
                const latest: any = await Message.findOne({
    ...
    async createGroup(userId: string, name: string, memberIds: string[]) {
        ...
        if (uniqueIds.length < 3)
            throw new Error('A group needs at least two friends')
        const friends = await Friendship.find({
            status: 'accepted',
            $or: [
                { requesterId: owner, addresseeId: { $in: uniqueIds } },
                { addresseeId: owner, requesterId: { $in: uniqueIds } },
            ],
        }).lean()
        ...
        if (
            uniqueIds
                .slice(1)
                .some((member) => !friendIds.has(member.toString()))
        )
            throw new Error('Only friends can be added to a group')
        return serialize(
            await Conversation.create({
                participantIds: uniqueIds,
                isGroup: true,
                name: String(name || 'Group chat')
                    .trim()
                    .slice(0, 120),
                ownerId: owner,
            }),
    ...
    async addGroupMembers(
        ...
        const friends = await Friendship.find({
            status: 'accepted',
            $or: [
                { requesterId: oid(userId), addresseeId: { $in: ids } },
                { addresseeId: oid(userId), requesterId: { $in: ids } },
            ],
        }).lean()
        const friendIds = new Set(
            friends.map((friend: any) =>
        ...
        if (ids.some((member) => !friendIds.has(member.toString())))
            throw new Error('Only friends can be added to a group')
        conversation.participantIds = [
            ...new Set([
                ...conversation.participantIds.map((id: any) => id.toString()),
    ...
    async sendMessage(
        ...
        recipients.forEach((recipient: any) =>
            emitToUser(recipient.toString(), 'message:new', serialize(message)),
        )
