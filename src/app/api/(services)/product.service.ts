...
export class ProductService {
    ...
    static async getAll(filters?: Record<string, any>) {
        ...
        // Length
        if (
            filters?.lengthMin !== undefined ||
            filters?.lengthMax !== undefined
        ) {
        ...
        if (filters?.sortBy) {
            switch (filters.sortBy) {
                ...
                case "soldCount":
                    sortObject = {
                        soldCount: filters.sortOrder === "asc" ? 1 : -1,
                    }
                    break
                case "rating":
                    sortObject = {
                        rating: filters.sortOrder === "asc" ? 1 : -1,
                    }
                    break
                case "name":
    ...
    static async getFilterOptions(
        ...
        try {
            ...
            const specifications = (result.specifications || []).map(
                (spec: any) => ({
                    ...spec,
            ...
            return {
                ...
                dimensionRanges: {
                    length: result.dimensionLength[0] || {
                        min: 0,
                        max: 0,
                        hasValue: 0,
                    },
                    width: result.dimensionWidth[0] || {
                        min: 0,
                        max: 0,
                        hasValue: 0,
                    },
                    height: result.dimensionHeight[0] || {
                        min: 0,
                        max: 0,
                        hasValue: 0,
                    },
