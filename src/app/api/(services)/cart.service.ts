...
export class CartService {
    ...
    static async addItem(
        ...
        const existingItemIndex = cart.items.findIndex(
            (item: ICartItem) => item.productId.toString() === data.productId,
        )
    ...
    static async removeMultipleItems(userId: string, itemIds: string[]) {
        ...
        cart.items = cart.items.filter(
            (item: ICartItem) => !itemIds.includes(item._id?.toString() || ""),
        )
    ...
    static async getCartStats(userId: string) {
        ...
        const outOfStockCount = validated.items.filter(
            (item: any) => !item.inStock,
        ).length
