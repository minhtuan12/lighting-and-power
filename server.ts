...
app.prepare().then(() => {
    ...
    io.use((socket, nextMiddleware) => {
        try {
            ...
            const user = jwt.verify(decodeURIComponent(token), process.env.ACCESS_TOKEN_SECRET!) as { userId?: string; id?: string }
            socket.data.userId = user.userId || user.id
            if (!socket.data.userId) return nextMiddleware(new Error('Unauthorized'))
            nextMiddleware()
        } catch { nextMiddleware(new Error('Unauthorized')) }
    })
